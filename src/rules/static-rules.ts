...
export function evaluateStaticRules(cfg: RuleConfig, ctx: { method: string; from?: string; to?: string; gasPriceWei?: bigint; gasLimit?: bigint }): RuleResult | null {
	...
	const addresses = [ctx.from?.toLowerCase(), ctx.to?.toLowerCase()].filter(Boolean) as string[];
